...
impl Default for Localstack {
    fn default() -> Self {
        ...
        env_vars.insert(
            "EAGER_SERVICE_LOADING".to_owned(),
            "1".to_owned(),
        );
        env_vars.insert(
        ...
        env_vars.insert(
            "PORT_WEB_UI".to_owned(),
            "9888".to_owned(),
        );
        env_vars.insert(
            "LAMBDA_REMOTE_DOCKER".to_owned(),
            "false".to_owned(),
        );
        env_vars.insert(
            "HOST_TMP_FOLDER".to_owned(),
            "/tmp/localstack".to_owned(),
        );
...
impl Image for Localstack {
    ...
    fn tag(&self) -> String { TAG.to_owned() }

    fn ready_conditions(&self) -> Vec<WaitFor> {
        vec![WaitFor::message_on_stdout("Execution of \"preload_services\" took")]
    }
