    ...
    async fn store(&self, converted_product: &ConvertedProduct) -> bool {
        if self.has_changes(converted_product).await {
            self.dynamo_db_client
                ...
                .item(
                    "id",
                    AttributeValue::S(converted_product.product_id.to_string()),
                )
                .item(
                    "data",
                    AttributeValue::S(serde_json::to_string(&converted_product).unwrap()),
                )
                .send()
                .await
                .expect(&format!(
                    "Could not store product {} in DynamoDB!",
    ...
    async fn delete(&self, product_id: &str) -> bool {
        if self.get_by_id(product_id).await.is_some() {
            self.dynamo_db_client
                ...
                .await
                .expect(&format!(
                    "Could not delete product {} from DynamoDB!",
                    &product_id
                ));
            true
        } else {
...
impl DynamoDbStoreConvertedProductService {
    async fn has_changes(&self, converted_product: &ConvertedProduct) -> bool {
        ...
        if attribute_value.is_some() {
            ...
            let stored_product: ConvertedProduct =
                serde_json::from_str(strin).expect("Could not parse product");
    ...
    async fn get_by_id(&self, product_id: &str) -> Option<AttributeValue> {
        match self
            .dynamo_db_client
            .get_item()
            .table_name(&self.table_name)
