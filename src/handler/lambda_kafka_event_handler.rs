...
impl LambdaKafkaEventHandler {
    pub async fn handle_lambda_kafka_event(&self, lambda_event: LambdaEvent<KafkaEvent>) -> Result<Value, Error> {
        info!(self.log, "Handle kafka event ...");
        let kafka_event = lambda_event.payload;
        self.save_converted_product_use_case.execute(lambda_kafka_event_controller(kafka_event)).await;
