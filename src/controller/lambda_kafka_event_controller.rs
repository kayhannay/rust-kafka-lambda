...
pub fn lambda_kafka_event_controller(kafka_event: KafkaEvent) -> Vec<ProductEvent> {
    ...
    kafka_event.records.iter().flat_map(|it| it.1)
        .map(|record| {
            let mut product: Option<Product> = None;
            if record.value.is_some() {
                let json_string = record.value.as_ref().unwrap().decode().expect("Could not decode event data");
                product = serde_json::from_str(&json_string).ok();
            }
            ProductEvent {
                product_id: record.key.clone().expect("No product id provided as key in the KafkaEvent!").decode().expect("Could not decode event key"),
                product
            }
        })
